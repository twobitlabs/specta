...
void spt_describe(NSString *name, void (^block)(void)) {
  spt_defineDescribeBlock(name, NO, block);
}
...
void spt_context(NSString *name, void (^block)(void)) {
  describe(name, block);
}
...
void spt_it(NSString *name, void (^block)(void)) {
  spt_defineItBlock(name, nil, 0, NO, block);
}
...
void spt_beforeAll(void (^block)(void)) {
  SPTReturnUnlessBlockOrNil(block);
  [SPTCurrentGroup addBeforeAllBlock:block];
}
...
void spt_afterAll(void (^block)(void)) {
  SPTReturnUnlessBlockOrNil(block);
  [SPTCurrentGroup addAfterAllBlock:block];
}
...
void spt_beforeEach(void (^block)(void)) {
  SPTReturnUnlessBlockOrNil(block);
  [SPTCurrentGroup addBeforeEachBlock:block];
}
...
void spt_afterEach(void (^block)(void)) {
  SPTReturnUnlessBlockOrNil(block);
  [SPTCurrentGroup addAfterEachBlock:block];
}
